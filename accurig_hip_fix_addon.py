bl_info = {
    ...
    "description": "Fixes the Hip bone Head/Tail X/Y alignment and Roll for AccuRIG / CC-compatible skeletons.",
    "warning": "",
    "category": "Rigging",
}
...
def fix_accurig_hip(context):
    ...
    before_head_x = hip_bone.head.x
    before_head_y = hip_bone.head.y
    before_tail_x = hip_bone.tail.x
    before_tail_y = hip_bone.tail.y
    before_roll = hip_bone.roll

    # Fix Hip bone:
    # - Set Head X/Y to 0
    # - Set Tail X/Y to 0
    # - Set Roll to 0
    # Z positions are preserved.
    hip_bone.head.x = 0.0
    hip_bone.head.y = 0.0
    hip_bone.tail.x = 0.0
    hip_bone.tail.y = 0.0
    ...
    after_head_x = hip_bone.head.x
    after_head_y = hip_bone.head.y
    after_tail_x = hip_bone.tail.x
    after_tail_y = hip_bone.tail.y
    after_roll = hip_bone.roll
    ...
    message = (
        ...
        f"Head X/Y: ({before_head_x:.6f}, {before_head_y:.6f}) "
        f"-> ({after_head_x:.6f}, {after_head_y:.6f}), "
        f"Tail X/Y: ({before_tail_x:.6f}, {before_tail_y:.6f}) "
        f"-> ({after_tail_x:.6f}, {after_tail_y:.6f}), "
        f"Roll: {before_roll:.6f} -> {after_roll:.6f}"
    )
...
class GECB_OT_FixAccuRIGHip(bpy.types.Operator):
    ...
    bl_description = "Sets the AccuRIG Hip bone Head/Tail X/Y to 0 and Roll to 0 for CC-compatible skeletons"
